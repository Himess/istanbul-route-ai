  ...
  async verifyPayment(txHash: string): Promise<VerificationResult> {
    ...
    try {
      ...
      if (receipt.to?.toLowerCase() !== config.contractAddress.toLowerCase()) {
        return { valid: false, error: "Transaction not sent to IstanbulRoute contract" };
      }
  ...
  async verifyParkingPayment(txHash: string): Promise<ParkingVerificationResult> {
    ...
    try {
      ...
      if (receipt.to?.toLowerCase() !== config.parkingContractAddress.toLowerCase()) {
        return { valid: false, error: "Transaction not sent to IstanbulRoute Parking contract" };
      }
