...
export async function calculateRoute(
  ...
  if (osrmResult.error || osrmResult.routes.length === 0) {
    ...
    console.warn(
      `[IstanbulRoute] OSRM unavailable (${osrmResult.error || "no routes"}), using grid fallback`,
    );
    return calculateRouteGridFallback(from, to, simulator);
  }

  // Gather live vehicle data for scoring — merge municipal simulator fleet
  // with real IETT buses from IBB SOAP API. Real buses reflect actual traffic.
  const simVehicles = simulator.getVehicles().map((v) => ({
    id: v.id,
    lat: v.lat,
    lng: v.lng,
    speed: v.speed,
  ...
  const iettBuses = await ibbClient.getBusPositions().catch(() => []);
  const realBuses = iettBuses.map((b) => ({
    id: `iett-${b.id}`,
    lat: b.lat,
    lng: b.lng,
    speed: b.speed,
  ...
  const vehicles = [...simVehicles, ...realBuses];

  // Use real IBB traffic index for city-wide congestion if available;
  // fall back to simulator zone congestion otherwise.
  const realTrafficIndex = await ibbClient.getTrafficIndex().catch(() => []);
  const realCityCongestion =
    realTrafficIndex.length > 0 ? realTrafficIndex[0].congestion : null;

  const zoneCongestion = getAllZoneCongestion().map((z) => ({
    zone: z.zone,
    // Blend simulator zone congestion with real city-wide reading (70% real / 30% sim)
    congestion: realCityCongestion !== null
      ? 0.3 * z.congestion + 0.7 * realCityCongestion
      : z.congestion,
  }));

  const congestionData = zoneCongestion;
  ...
  if (osrmResult.routes.length === 1) {
    ...
    try {
      ...
      const resp = await fetch(detourUrl, {
        ...
        headers: { "User-Agent": "IstanbulRoute-Istanbul/1.0" },
      });
