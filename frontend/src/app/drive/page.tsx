...
import { AgentRationaleCard } from "@/components/Drive/AgentRationaleCard";
...
export default function DrivePage() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#0A0F1C] relative">
          ...
          <div className="max-w-[402px] mx-auto bg-[#1E293B] rounded-t-[12px] p-5 space-y-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-[8px] bg-[#22D3EE]/[0.06] border border-[#22D3EE]/20">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-white text-[13px] font-medium">IstanbulRoute route</span>
      ...
      {/* ─── COMPARISON: Pencil exact + Agent rationale ─── */}
      {phase === "comparison" && paidRoute && (
        <div className="absolute bottom-[95px] left-0 right-0 z-[900] slide-up">
          <div className="max-w-[402px] mx-auto bg-[#1E293B] rounded-t-[12px] p-5 space-y-3">
            ...
            <div className="flex items-center justify-between p-3 rounded-[8px] bg-[#22D3EE]/[0.06] border border-[#22D3EE]/20">
              <div className="flex items-center gap-2.5"><div className="w-[10px] h-[10px] rounded-full bg-[#22D3EE]" /><span className="text-white text-[13px] font-medium">IstanbulRoute route</span></div>
              <span className="font-mono text-[15px] font-bold text-[#22D3EE]">{paidRoute.optimizedTime} min</span>
            </div>
            <div className="flex items-center justify-center gap-3 py-2.5 rounded-[8px] bg-[#22D3EE]/[0.04] border border-[#22D3EE]/10 flex-wrap">
              <span className="font-mono text-[14px] font-bold text-[#22D3EE]">
                {paidRoute.savedMinutes > 0
                  ? `${paidRoute.savedMinutes} min faster`
                  : paidRoute.savedMinutes < 0
                  ? `${Math.abs(paidRoute.savedMinutes)} min slower (safer)`
                  : "Similar timing"}
              </span>
              <span className="text-[#475569]">&middot;</span>
              <span className="text-[11px] text-[#94A3B8]">{paidRoute.vehiclesUsed?.length || 0} vehicles</span>
              <span className="text-[#475569]">&middot;</span>
              <span className="font-mono text-[11px] text-[#22D3EE]">${paidCost} paid</span>
            </div>

            {/* Agent rationale — collapsible */}
            {paidRoute.agent && <AgentRationaleCard agent={paidRoute.agent} />}
